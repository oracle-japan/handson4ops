...
|作業者 |作業目的   |
|:---   |:---       |
|運用者(Ops)|開発者が作成したアプリケーションを実行環境(ACCS,DBCS)にデプロイするためのスクリプト群を作成し、Developer Cloud上のGitリポジトリへのプッシュを行います。|

+ 左ペインの表示をProjectからFileに変更する

![](./img2/2-1create_shell.PNG)

+ スクリプト群のファイルは、このハンズオンマテリアルと同じリポジトリに配置してあります。リポジトリに[アクセス](https://github.com/oracle-japan/handson4ops)して、ファイル一式をダウンロードします。

![](./img2/2-6-0create_shell.PNG)

zipファイルを解凍の後、scriptsフォルダに入っているファイル群を、EmployeeRESTAppのshフォルダにコピーします。

shフォルダ

![](./img2/2-3create_shell.PNG)

この後、ハンズオン環境に合わせて、コピーしたファイルの編集を行います。ここまで作業が進んだところで、講師に声をかけてください。
